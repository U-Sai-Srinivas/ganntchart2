...
# Primary path: browser-side PNG via the chart's camera 📷 button — no server packages
# needed. The modebar renders the image itself at the exact slide dimensions below.
st.markdown("**Save a slide-sized PNG** — hover the preview and click the camera 📷 icon "
            "(top-right). It downloads at exactly the size above, rendered in your browser — "
            "no extra install required.")
export_config = {
    "toImageButtonOptions": {"format": "png", "filename": f"gantt_chart_{exp_w}x{exp_h}",
                             "width": exp_w, "height": exp_h, "scale": 2},
    "displaylogo": False,
    "modeBarButtonsToRemove": ["lasso2d", "select2d", "autoScale2d"],
}
with st.expander("🖼️ Show export preview (click the 📷 camera to download PNG)", expanded=True):
    st.plotly_chart(ppt_fig, use_container_width=True, theme=None, config=export_config,
                    key="ppt_preview")

col_exp1, col_exp2 = st.columns([1, 1])

# Bonus path: if kaleido IS installed, offer a true one-click server-side download too.
with col_exp1:
    if st.button("⚙️ One-click PNG (needs kaleido)", use_container_width=True):
        try:
            ...
            st.session_state.pop('ppt_png', None)
            st.info("`kaleido` isn't installed — use the camera 📷 button above instead, "
                    "or run `pip install -U kaleido` to enable one-click download.")
...
# Always-available fallback: self-contained interactive HTML (opens in any browser,
# print-to-PDF or screenshot at any size). Never depends on kaleido.
with col_exp2:
    html_bytes = ppt_fig.to_html(include_plotlyjs="cdn", full_html=True).encode("utf-8")
    st.download_button("⬇️ Download interactive HTML", html_bytes, "gantt_chart.html",
