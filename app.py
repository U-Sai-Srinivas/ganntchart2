...
st.session_state.task_data = edited_df.copy()  # persist edits back so they survive reruns
...
fig.update_layout(
    xaxis_title="Timeline", yaxis_title="", height=max(400, len(valid_df) * 45),
    margin=dict(l=140 if group_mode == "Swimlanes by Resource" else 20, r=20, t=40, b=20),
    showlegend=True, paper_bgcolor=bg_color, plot_bgcolor=bg_color, font=dict(color=text_color),
)
fig.update_xaxes(tickfont=dict(size=axis_tick_size), title_font=dict(size=axis_title_size))
fig.update_yaxes(tickfont=dict(size=task_label_size), title_font=dict(size=axis_title_size))
...
def build_ppt_figure(base_fig, title_size, tick_size, label_size, n_rows, labels, is_swimlane):
    """Clone the chart at a fixed high-res canvas with legibility floors, so it drops into a PPT slide cleanly."""
    export_fig = go.Figure(base_fig)
    title_size, tick_size, label_size = max(title_size, 18), max(tick_size, 14), max(label_size, 13)
    max_label_len = max((len(str(t)) for t in labels), default=10)
    left_margin = min(480, max(160, int(max_label_len * label_size * 0.62))) + (60 if is_swimlane else 0)
    export_fig.update_xaxes(tickfont=dict(size=tick_size), title_font=dict(size=title_size))
    export_fig.update_yaxes(tickfont=dict(size=label_size), title_font=dict(size=title_size))
    export_fig.update_layout(
        width=1920, height=max(1080, n_rows * max(50, label_size * 4) + 220),
        margin=dict(l=left_margin, r=80, t=100, b=100),
...
if generate_clicked:
    try:
        ppt_fig = build_ppt_figure(fig, axis_title_size, axis_tick_size, task_label_size,
                                    len(valid_df), display_order, group_mode == "Swimlanes by Resource")
...
if 'ppt_png' in st.session_state:
    with col_exp2:
        st.download_button("⬇️ Download PPT PNG (1920px wide)", st.session_state['ppt_png'],
                            "gantt_chart_ppt.png", "image/png", use_container_width=True)
