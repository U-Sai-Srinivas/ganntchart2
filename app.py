...
with st.sidebar:
    ...
    show_row_guides = st.checkbox("Row guides (zebra stripes + dotted lines)", value=True,
                                  help="Shades alternate rows and draws a faint dotted line from each label to its bar.")
...
# Row guides: zebra stripes + a faint dotted line at each row tie the label to its bar
if show_row_guides:
    if group_mode != "Swimlanes by Resource":  # swimlanes already have their own banding
...
fig.update_layout(
    xaxis_title="Timeline", yaxis_title="", height=max(400, len(valid_df) * row_height),
